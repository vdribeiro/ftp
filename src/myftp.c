...
int datasocket_create_PORT(int socketc){
	...
	//getsockname (sd, (SA *)&name,&length);
	//printf("Server Port is: %d\n", ntoaddr(server.sin_data));
...
void cmd_help() {
	...
	printf("MGET - copy multiple files from the current remote machine to the local machine\n");
	printf("PUT - copy one file from the current local machine to the remote machine\n");
	printf("MPUT - copy multiple files from the current local machine to the remote machine\n");
	printf("TRANSFER - transfer one file from the current remote machine to the other\n");
	printf("MTRANSFER - transfer multiple files from the current remote machine to the other\n");
...
int mtransfer(int socketc1, int socketc2, char * filepath) {
	...
	//big vai conter a lista completa de ficheiros na pasta
	//a seguir dividimos a lista por linhas
	n = divide(big, "\r\n" , &linhas);
	if (n<0) return -1;

	for(counter=0; counter<n; counter++) {
		//dividimos linhas por palavras
		divide(linhas[counter], " ", &words);
